...
if (-not (Test-Path -LiteralPath "README.md" -PathType Leaf)) {
    throw "README.md not found in repository root"
}

if (-not (Test-Path -LiteralPath "LICENSE" -PathType Leaf)) {
    throw "LICENSE not found in repository root"
}
...
try {
    ...
    $actualEntries = Get-ChildItem -LiteralPath $validationDirectory -File |
