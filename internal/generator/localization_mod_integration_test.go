...
func TestGenerateUsesActiveLocalizationModAsMainSource(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(modDir, "mod.manifest"), []byte(`<?xml version="1.0"?><kcd_mod><info><name>Russian Fix</name><modid>russian_fix</modid></info></kcd_mod>`), 0o600); err != nil {
		t.Fatal(err)
	}
